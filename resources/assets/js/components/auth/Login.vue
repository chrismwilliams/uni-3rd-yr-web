<template>
  <form class="form" method="POST" action="/login" @submit="checkForm" @keydown="form.errors.clear($event.target.name); form.errors.clear('credentials')">
      <input type="hidden" name="_token" :value="this.$root.token">
      <h1>Login</h1>
      <div class="field">
        <label class="label" for="email">E-Mail</label>
        <div class="control has-icons-left">
          <input class="input" :class="{ 'is-danger': (form.errors.has('email') || form.errors.has('credentials')) }"  id="email" ref="email" name="email" type="email" placeholder="Email" v-model.trim="form.email" required>
          <span class="icon is-small is-left">
            <i class="fas fa-envelope"></i>
          </span>
        </div>
        <p v-if="form.errors.has('credentials')" class="help is-danger" v-text="form.errors.get('credentials')"></p>
        <p v-if="form.errors.has('email')" class="help is-danger" v-text="form.errors.get('email')"></p>
      </div>
      
      <div class="field">
        <label class="label" for="password">Password</label>
        <div class="control has-icons-left">
          <input class="input" :class="{ 'is-danger': form.errors.has('password') }" id="password" name="password" type="password" v-model.trim="form.password" placeholder="Password" required>
          <span class="icon is-small is-left">
            <i class="fas fa-lock"></i>
          </span>
        </div>
        <p v-if="form.errors.has('password')" class="help is-danger" v-text="form.errors.get('password')"></p>
      </div>

      <div class="field flex">
        <label class="checkbox">
          <input type="checkbox" name="remember"> Remember Me
        </label>
        <a class="pass_link" href="/password/reset">Forgot Your Password?</a>
      </div>

      <div class="field">
        <div class="control">
          <button class="button is-primary" type="submit">Login</button>
        </div>
      </div>
    </form>
</template>

<script>
import Form from "../../Form";
import { validateEmail } from "../../mixins";

export default {
  name: "Login",
  props: ["old", "errors"],
  mixins: [validateEmail],
  data() {
    let loginForm = new Form(
      {
        email: "",
        password: ""
      },
      this.old
    );
    loginForm.errors.record(this.errors);

    return {
      form: loginForm
    };
  },
  methods: {
    checkForm(e) {
      let { email } = this.form.data();

      if (email.length < 6 || !this.checkEmail(email)) {
        this.form.errors.recordOne({
          name: "email",
          message: "Please enter a valid email address"
        });
      }

      if (this.form.isSet() && this.form.errors.any()) {
        e.preventDefault();
      }
    }
  }
};
</script>
